...
bekk.each = (function () {
  ...
  function unEnumerated(object, properties) {
    ...
    for (var prop in object) {
      if (bekk.isOwnProperty(object, prop)) {
        enumerated -= 1;
        object[prop] = false;
      }
    }
  ...
  return function (object, callback) {
    ...
    if (fixes) {
      ...
      for (var i = 0, l = fixes.length; i < l; i++) {
        ...
        if (bekk.isOwnProperty(object, property)) {
...
bekk.extend = (function () {
  function extend(target, source) {
    ...
    bekk.each(source, function (prop, val) {
      target[prop] = val;
    });
